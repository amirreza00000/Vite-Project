...
const SmallNews = ({imgSrc,detail,title,id}) => {
  return (
    <Link to={"/blog/Detail/"+id} className="bg-[#F3F4F6] min-h-8 w-68 rounded-3xl *:relative *:bottom-10">
      <img
        src={!imgSrc?"/newspaper.png":imgSrc}
        alt="no image"
        className="h-60 w-60 rounded-3xl mx-auto"
      />
      <h3 className="px-5 my-2 text-end overflow-hidden text-ellipsis">{title}</h3>
